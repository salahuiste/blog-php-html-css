		...
		header('Location: login.php');
			...
			try {
				...
				if($_SERVER["REQUEST_METHOD"] == "POST") {
					$pat=NULL;
					$pre=$bdd->prepare('UPDATE tbl_articles SET id=:idn,title=:titlen,description=:descrn,date_article=:daten,image=:imagen where id=:idn');
...
<?php 
		$ge=$bdd->query('SELECT title,description FROM tbl_articles WHERE id=\''.$_GET['id'].'\'');
		$donnees=$ge->fetch();
?>
	<form action="<?php echo "edit_article.php?id=".$_GET['id']; ?>" method="post" enctype="multipart/form-data">
	<table align="center">
		<tr>

...
<body>
<div>
<h2 id='title_page'>Les articles:</h2>
</div>
<div>
	<?php 
		$np;
		if(empty($_GET['page']) or $_GET['page']==1)
			...
			$np=$_GET['page'];
		$np*=3;
		$ge=$bdd->query('SELECT * FROM tbl_articles LIMIT '.$np.',6');
				...
				for($i=0;$i<3;$i++) {
					...
					echo "<div class='text_img'>";
							...
							echo "<div class='text_lire'>";
									...
									echo "</div>";
									echo "<div class='btt'>";
										echo "<a  href='articles_detail.php?id=".$donnees['id']."'>La suite</a>";
									echo "</div>";	
							echo"</div>";
							echo "</div>";
						echo "</div>";
...
<div align="right">
	<?php 
		$getnm=$bdd->query('SELECT count(*)as nb_page FROM tbl_articles');
